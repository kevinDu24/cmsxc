...
public class CommonUtil {
	...
	public void setDataSource(DataSource dataSource) {
		CommonUtil.dataSource = dataSource;
	}
...
//		double dd = Double.parseDouble(d);
	...
	/**
	 * 将long型数据转换为固定位数的字符串（数据不够左侧补0）
	 * @param number 数据
	 * @param num 字符串位数
	 * */
	public static String getNumberStrLeftPatch(Long number, int num) {
		if (number != null) {
			if (number > 0 && num > 0) {
	 ...
	 * @return
	 */
	public static List<String> breakStringByComma(String str){
		List<String> stringList = new ArrayList<>();
		if(StringUtil.isNull(str)){
			return stringList;
		}
		String[] strs = str.split(Constants.COMMA);
		for(int i = 0; i < strs.length; i++){
