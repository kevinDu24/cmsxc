...
import cn.com.leadu.cmsxc.appuser.service.FileService;
...
public class FileServiceImpl implements FileService{
    ...
    public ResponseEntity<RestResponse> uploadFile(String type,MultipartFile file){
        ...
        if("apply".equals(type)){
            fileUrl = FileUtil.saveFile(nameName, fileUploadProperties.getAuthorizationPath() + uploadDate + "/", file, fileUploadProperties.getRequestAuthorizationPath() + uploadDate + "/" );
            ...
            fileUrl = FileUtil.saveFile(nameName, fileUploadProperties.getRecordPath() + uploadDate + "/", file, fileUploadProperties.getRequestRecordPath() + uploadDate + "/" );
        }else if("recoveryfinish".equals(type)){
            fileUrl = FileUtil.saveFile(nameName, fileUploadProperties.getRecoveryfinishPath() + uploadDate + "/", file, fileUploadProperties.getRequestrecoveryFinishPath() + uploadDate + "/" );
        }else{
    ...
    public void downloadFile(HttpServletResponse response,String type,String fileName, String loadDate) {
        ...
        String path = "";
        if("apply".equals(type)){
            path = fileUploadProperties.getAuthorizationPath() + loadDate + "/" + fileName;
        }else if("photo".equals(type)){
            path = fileUploadProperties.getPhotoPath() + loadDate + "/" + fileName;
        }else if("record".equals(type)){
            path = fileUploadProperties.getRecordPath() + loadDate + "/" + fileName;
        }else if("recoveryfinish".equals(type)){
            path = fileUploadProperties.getRecoveryfinishPath() + loadDate + "/" + fileName;
        }
