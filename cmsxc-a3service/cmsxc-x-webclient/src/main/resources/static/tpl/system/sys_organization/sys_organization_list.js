 ...
 */
app.controller('sys_organization_list_controller', ['$scope', '$http', '$modal', 'toaster','$compile', function ($scope, $http, $modal, toaster,$compile) {
    ...
    $scope.dataTableProperties= {
        ...
        //table的html id
        dataTableId:'sys_organization_table',
        //table的列
        dataTableColumn: [
            defaultCheckBox(),
    ...
    // $http.get('sys_organization/findSysOrganizationByTree').success(function(data){
    //
    ...
    //             $scope.orgParentId = data.nodeId;
    //             $scope.resetSysOrganization();
    //         }
    //     });
    //
    // });
    ...
    $scope.manageSysOrganization = function(){
        var rowsIds =  $scope.dataTable.getRowsIds();
        if(rowsIds.length < 1)
            modalAlert($modal,'请您选择需要管理的组织机构！');
        else if(rowsIds.length > 1)
            modalAlert($modal,'只能同时管理一条组织机构！');
        else{

            var rtn = $modal.open({
                backdrop : 'static',
                size:'lg',
                templateUrl: 'tpl/system/sys_organization/sys_organization_list_manage.html'+getCacheTime(),
                controller: 'sys_organization_list_manage_controller',
                resolve:{
                    sysOrg : function (){ return $scope.dataTable.getRow() }
                }
            });
            rtn.result.then(function (status) {
