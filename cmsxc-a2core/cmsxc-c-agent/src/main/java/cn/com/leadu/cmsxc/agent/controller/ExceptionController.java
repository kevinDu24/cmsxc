...
package cn.com.leadu.cmsxc.agent.controller;
...
public class ExceptionController implements ErrorController {
	...
	public ResponseEntity error400(HttpServletRequest request) {
		...
		return new ResponseEntity<>(
				RestResponseGenerator.genFailResponse(ResponseFailEnum.RESOURCE_ACCESS_ERROR,body, getMessage("请求参数错误", code)), HttpStatus.OK);
	...
	public ResponseEntity error401(HttpServletRequest request) {
		...
		String code = get4RandomCode();
		return new ResponseEntity<>(
				RestResponseGenerator.genFailResponse(ResponseFailEnum.OAUTH2_ERROR,body,getMessage("权限异常", code)), HttpStatus.OK);
	}
	...
	public ResponseEntity error404(HttpServletRequest request) {
		...
		String code = get4RandomCode();
		return new ResponseEntity<>(
				RestResponseGenerator.genFailResponse(ResponseFailEnum.RESOURCE_ACCESS_ERROR,body,getMessage("资源未找到", code)), HttpStatus.OK);
	}
	...
	public ResponseEntity error405(HttpServletRequest request) {
		...
		String code = get4RandomCode();
		return new ResponseEntity<>(
				RestResponseGenerator.genFailResponse(ResponseFailEnum.RESOURCE_ACCESS_ERROR,body,getMessage("请求类型错误", code)), HttpStatus.OK);
	}
	...
	public ResponseEntity error500(HttpServletRequest request) {
		...
		if(body.containsKey("message") && body.containsKey("exception")){
			if(body.get("message").toString().equals("400 ")||
				...
				return new ResponseEntity<>(
						RestResponseGenerator.genFailResponse(ResponseFailEnum.TOKEN_INVALID, body,getMessage("无效token", code)), HttpStatus.OK);
			}
			if(body.get("message").toString().equals("GENERAL")){
				return new ResponseEntity<>(
						RestResponseGenerator.genFailResponse(ResponseFailEnum.RESOURCE_ACCESS_ERROR, body,getMessage("服务状态异常", code)), HttpStatus.OK);
			}
		}
		return new ResponseEntity<>(
				RestResponseGenerator.genFailResponse(ResponseEnum.FAILURE,body,getMessage("请求失败", code)), HttpStatus.OK);
